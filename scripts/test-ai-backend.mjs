const baseUrl = (process.env.MACHTOBS_AI_API_URL || 'http://localhost:5173').replace(/\/+$/, '');
...
async function post(pathname, body) {
  const response = await fetch(`${baseUrl}${pathname}`, {
    ...
    headers: {
      ...
      'X-Machtobs-Install-Id': '123e4567-e89b-42d3-a456-426614174000',
    },
...
try {
  console.log(`Testing Machtobs AI backend at ${baseUrl}`);
  ...
  console.error('Run pnpm dev locally or set MACHTOBS_AI_API_URL to another backend URL.');
  process.exitCode = 1;
}
