...
function configureRemoteProvider() {
  ...
  vi.stubEnv('VERCEL', '1');
  vi.stubEnv('MACHTOBS_AI_DAILY_LIMIT', '20');
  vi.stubEnv('MACHTOBS_ALLOW_MEMORY_RATE_LIMIT', 'false');
...
describe('distributed rate limit', () => {
  ...
  test('explicit local memory fallback enforces the configured limit', async () => {
    ...
    vi.stubEnv('VERCEL', '');
    vi.stubEnv('MACHTOBS_ALLOW_MEMORY_RATE_LIMIT', 'true');
    vi.stubEnv('MACHTOBS_AI_DAILY_LIMIT', '1');
    const { checkRateLimit } = await import('./rate-limit');
