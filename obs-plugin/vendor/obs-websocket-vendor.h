...
#ifndef MACHTOBS_OBS_WEBSOCKET_VENDOR_H
#define MACHTOBS_OBS_WEBSOCKET_VENDOR_H
...
static proc_handler_t *machtobs_websocket_proc_handler;

static inline proc_handler_t *machtobs_websocket_get_proc_handler(void)
{
...
static inline bool machtobs_websocket_ensure_proc_handler(void)
{
  if (!machtobs_websocket_proc_handler)
    machtobs_websocket_proc_handler = machtobs_websocket_get_proc_handler();
  return machtobs_websocket_proc_handler != NULL;
}

static inline obs_websocket_vendor obs_websocket_register_vendor(const char *vendor_name)
{
  if (!machtobs_websocket_ensure_proc_handler())
    return NULL;

  calldata_t calldata = {0};
  calldata_set_string(&calldata, "name", vendor_name);
  proc_handler_call(machtobs_websocket_proc_handler, "vendor_register", &calldata);
...
static inline bool obs_websocket_vendor_register_request(
  ...
  void *private_data)
{
  if (!machtobs_websocket_ensure_proc_handler() || !vendor || !request_type ||
  ...
  proc_handler_call(machtobs_websocket_proc_handler, "vendor_request_register", &calldata);
  ...
  const char *request_type)
{
  ...
  calldata_set_string(&calldata, "type", request_type);
  calldata_set_ptr(&calldata, "vendor", vendor);
  proc_handler_call(machtobs_websocket_proc_handler, "vendor_request_unregister", &calldata);
  bool result = calldata_bool(&calldata, "success");
  calldata_free(&calldata);
  return result;
}
